...
class ServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object ob) throws Exception {
        IMContent msg = getJsonIMessage(ob);
        if (msg == null)
            return;
    ...
    //握手认证
    private void personAuthentication(ChannelHandlerContext ctx, final IMContent msg) {
        ...
        imMessage.Entity = "";
        imMessage.ReceiveID = ctx.channel().id().toString();
        imMessage.Flag = true;
        imMessage.Msg = "登录成功";
    ...
    public IMContent getJsonIMessage(Object msg) {
        try {
            ...
            return new Gson().fromJson(result, IMContent.class);
        } catch (Exception e) {
